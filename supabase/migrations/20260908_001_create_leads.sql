...
CREATE TABLE IF NOT EXISTS public.leads (
  ...
  CONSTRAINT leads_company_length CHECK (
    LENGTH(TRIM(company)) BETWEEN 2 AND 120
  ),

  CONSTRAINT leads_email_length CHECK (
    LENGTH(TRIM(email)) BETWEEN 5 AND 254
  ),

  CONSTRAINT leads_area_length CHECK (
    LENGTH(TRIM(area)) BETWEEN 2 AND 100
  ),

  CONSTRAINT leads_comment_length CHECK (
    LENGTH(TRIM(comment)) BETWEEN 3 AND 2000
  ),
  ...
  CONSTRAINT leads_consent_consistency CHECK (
    (consent_privacy = true AND consent_at IS NOT NULL)
    OR
